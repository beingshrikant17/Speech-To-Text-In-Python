...
class SpeechRecognitionApp(tk.Tk):
    def __init__(self):
        ...
        self.scr = scrolledtext.ScrolledText(self, width=130, height=15)
        self.scr.insert(1.0,"Enter your input or Speak")
        self.scr.pack(pady=25)
        ...
        self.speak_button = RoundedButton1(self.button_frame, text="Speak", command=self.start_listening, bg = "dark blue", fg = "black", font=("Helvetica", 16),width=20, height=2)
        self.speak_button.grid(row=0, column=0, padx=10, pady=10)

        #Declaring the Text to voice Button
        self.TextToVoice_button = RoundedButton3(self.button_frame, text="Text to Voice", command=self.speak, bg = "#80669d", fg = "black", font=("Helvetica", 16),width=20, height=2)
        self.TextToVoice_button.grid(row=0, column=1, padx=10, pady=10)

        #Declaring the Copy Text Button
        self.copy_button = RoundedButton2(self.button_frame, text="Copy Text", command=self.copy_text, bg = "green", fg = "black", font=("Helvetica", 16),width=20, height=2)
        self.copy_button.grid(row=1, column=0, padx=10, pady=10)

        #Declaring the Change voice Button
        self.Change_Voice_button = RoundedButton4(self.button_frame, text="Clear Scrollbar", command=self.delete_scrollbar, bg = "gray", fg = "black", font=("Helvetica", 16),width=20, height=2)
        self.Change_Voice_button.grid(row=1, column=1, padx=10, pady=10)

        #Declaring the Exit Button
        self.exit_button = RoundedButton5(self.button_frame, text="Exit", command=self.exit_app, bg = "red", fg = "black", font=("Helvetica", 16),width=20, height=2)
    ...
    def speak(self):
        recognized_text=self.scr.get("1.0", END)
        text_to_speech(recognized_text, "Test.mp3")
